...
for (int i=0; i<_data.size(); i++)
{
    ...
    // SSyy
    _SSyy= _SSyy + pow(_data[i].second-_yAve, 2);
    // SSxy
    _SSxy= _SSxy + (_data[i].second-_yAve)*(_data[i].first-_xAve);
    // SSxx
    _SSxx= _SSxx + pow(_data[i].first-_xAve, 2);
}
...
{
    _stdDev= std::pow((_SSyy-_intercept*_SSxy)/(_data.size()-2) , 1/2);
}

// parahelium is the minimum distance;
...
void Regression::uncertaintiesCalculator()
{
    ...
    _uncertaintySlope = _stdDev/std::pow(_SSxx, 1/2);
